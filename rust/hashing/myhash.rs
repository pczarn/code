...
macro_rules! impl_hash {
    ($ty:ident, $uty:ident) => {
        impl<'a> Hash<'a> for &'a $ty {
            ...
            // type Contents = Cloned<slice::Iter<'a, u8>>;
            #[inline(always)]
            fn as_byte_contents(&self) -> (IntIter<'a>, usize) {// slice::IntoIter<u8> {
...
type SimpleContents<'a> = Chain<slice::Iter<'a, u8>, slice::Iter<'a, u8>>;

impl<'a> Hash<'a> for &'a [u8] {
    type Contents = SimpleContents<'a>;
    fn as_byte_contents(&self) -> (SimpleContents<'a>, usize) {
    ...
    fn next(&mut self) -> Option<u64> {
        ...
        match self.iter.next() {
            Some(mi) => {
                ...
                let mut msg: [u8; 8] = //unsafe { ::std::mem::uninitialized() };
                                         [*mi, 0, 0, 0, 0, 0, 0, 0];
                // msg[0] = mi;
                let mut iter = &mut self.iter;
                // for (i, x) in (1u..8).zip(&mut self.iter) {
                for i in 1u..8 {
                // for _ in 0..7 {
                    // mi <<= 8;
...
fn bench_slice(bench: &mut Bencher) {
    let val: &[u8] = &[12, 23, 34, 45, 12, 12, 23, 34, 45, 12, 12, 23, 34, 45, 12, 12, 23, 34, 45, 12, ];
...
fn bench_std(bench: &mut Bencher) {
    let mut val: &[u8] = &[12, 23, 34, 45, 12, 12, 23, 34, 45, 12, 12, 23, 34, 45, 12, 12, 23, 34, 45, 12, ];
    // let mut val: &[&[u8]] = &[&[12, 23, 34, 45, 12, 12, 23, 34, 45, 12, ], &[12, 23, 34, 45, 12, 12, 23, 34, 45, 12, ], &[12, 23, 34, 45, 12, 12, 23, 34, 45, 12, ]];
    // let mut val = 0u64;
    // let val = "abecadło";
    let mut r = &mut val;
    ...
    // println!("{}", ::std::hash::hash::<_, ::std::hash::SipHasher>(&r));
...
fn main() {
    ...
    let mut val: &[u8] = &[12, 23, 34,
                            // 12, 23, 34, 45, 12, 12, 23, 34, 45, 12,
                            // 12, 23, 34, 45, 12, 12, 23, 34, 45, 12,
                            ];
    // let mut val = 123456u64;
    let mut r = &val;
    unsafe { asm!("" : "+r"(r) ::: "volatile"); }
